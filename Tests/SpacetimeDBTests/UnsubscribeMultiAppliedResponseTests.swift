...
struct UnsubscribeMultiAppliedResponseTests {
    ...
    @Test func decodesUnsubscribeMultiAppliedCorrectly() throws {
        ...
        // QueryUpdate with empty deletes + empty inserts in the new
        // BsatnRowList format (size_hint tag + variant + dataSize).
        writer.write(UInt8(1))      // deletes hint = RowOffsets
        writer.write(UInt32(0))     // 0 offsets
        writer.write(UInt32(0))     // 0 data bytes

        writer.write(UInt8(0))      // inserts hint = FixedSize
        ...
        // QueryUpdate with empty deletes + empty inserts.
        writer.write(UInt8(1))      // deletes hint = RowOffsets
        writer.write(UInt32(0))     // 0 offsets
        writer.write(UInt32(0))     // 0 data bytes
