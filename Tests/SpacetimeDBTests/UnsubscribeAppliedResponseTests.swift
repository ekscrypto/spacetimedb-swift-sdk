...
struct UnsubscribeAppliedResponseTests {
    ...
    @Test func decodesUnsubscribeAppliedCorrectly() throws {
        ...
        // Add a simple CompressibleQueryUpdate (uncompressed). BsatnRowList
        // wire format: u8 size_hint tag (0=FixedSize(u16), 1=RowOffsets(u32
        // count + count*u64)) + u32 dataSize + dataSize bytes.

        writer.write(UInt8(0))      // Uncompressed tag

        // Deletes: empty via RowOffsets variant (count=0, dataSize=0)
        writer.write(UInt8(1))      // hint tag = RowOffsets
        writer.write(UInt32(0))     // 0 offsets
        writer.write(UInt32(0))     // 0 data bytes

        // Inserts: empty via FixedSize(0) (size=0, dataSize=0)
        writer.write(UInt8(0))      // hint tag = FixedSize
        writer.write(UInt16(0))     // fixed row size = 0
        writer.write(UInt32(0))     // 0 data bytes
